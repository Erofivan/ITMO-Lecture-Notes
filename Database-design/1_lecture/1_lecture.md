...
- Схема: у каждого узла два числа — left_ bound и right _bound. Для любого узла все потомки имеют bounds внутри его диапазона; обход «потомки» - WHERE left _bound > N AND right_bound < M.
...
- Кэш ответа АРІ сохранённый ответ вешнего сервиса.
