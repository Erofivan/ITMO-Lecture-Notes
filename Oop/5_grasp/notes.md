...
1. [Введение](#введение)
2. [Low Coupling & High Cohesion (Низкое зацепление и высокая связность)](#low-coupling--high-cohesion-низкое-зацепление-и-высокая-связность)
3. [Information Expert (Информационный эксперт)](#information-expert-информационный-эксперт)
4. [Creator (Создатель)](#creator-создатель)
5. [Controller (Контроллер)](#controller-контроллер)
6. [Object Indirection (Объектное посредничество)](#object-indirection-объектное-посредничество)
7. [Polymorphism (Полиформизм)](#polymorphism-полиморфизм)
8. [Protected Variations (Защита от изменений)](#protected-variations-защита-от-изменений)
9. [Pure Fabrication (Чистая выдумака)](#pure-fabrication-чистая-выдумка)
...
Принципы GRASP не являются догмой, которой всегда надо следовать — это скорее рекомендации, основанные на опыте проектирования систем. Они помогают создавать код с низким зацеплением (low coupling), высокой связностью (high cohesion) и понятной структурой.

## Low Coupling & High Cohesion (Низкое зацепление и высокая связность)
...
**Важно:** Cohersion переводят на русский, как *связность*, а coupling, либо как связан*ность*, либо как *зацепление*. Чтобы не путать coupling и cohesion, будем называть:
- **Coupling** — зацепление
- **Cohesion** — связность

Иначе (как вы можете встретить в интернете) будет не очень приятно читать внутри одного предложения что-то вроде: "связность и связанность". В русском языке слова похожи и, на мой взгляд, куда легче переводить Coupling именно как зацепление, а не связанность.

Однако помните о том, что, если вы встретите термин *связанность*, то под ним подразумевает именно *связанность* (coupling), а не *связность* (cohersion).
...
Аналогия с базой данных: у нас есть внешний ключ, через который мы взаимодействуем с другими таблицами, но остальные данные изолированы и они используются только в рамках одной таблицы. Классы и модули должны взаимодействовать также: через какой-то (или несколько) внешних ключей (методов), но не трогать *напрямую* другие данные.

Другими словами, классы и модули должны взаимодействовать между собой как бы "на краешках". Представьте себе двое мужчин. Они могут поздороваться, просто пожав друг другу руки (это будет Low Coupling - хорошо), а могут обняться и ещё ногами друг за друга зацепиться или вообще лечь в обнимку друг на друга, так сказать, прильнуть телом к телу (это будет High Coupling - плохо). При этом каждый из мужчин должен иметь свои, строго ограниченные обязанности и не делать чересчур много или чересчур мало.
...
Если класс хранит какую-то информацию, то именно он должен отвечать за операции, связанные с этой информацией. Не нужно выносить вычисления наружу, если их можно корректно реализовать внутри самого владельца данных.
...
4. **Проблемы с повторным использованием**: Если нужно получить стоимость заказа в другом месте программы, придётся либо дублировать логику (копипастить) расчёта, либо создавать неестественную зависимость от `ReceiptService`.
...
Принцип **Information Expert** учит распределять поведение так, чтобы каждый класс работал с собственной информацией, а не перекладывал ответственность на внешние сервисы. 
...
**Решение:** Как правило класс `B` должен отвечать за создание объектов `A`, если выполняется одно или несколько из следующих условий:
...
**Creator** естественно продолжает идею **Information Expert**: тот, кто знает достаточно, чтобы создать объект, и естественным образом с ним взаимодействует, должен отвечать за его создание.
...
> «Контроллер принимает входящие запросы и координирует их выполнение, делегируя работу другим объектам. Сам он бизнес-логику не реализует»
...
То есть контроллер — это переходник между моделями представления (UI, API) и моделями бизнес-логики (доменными объектами).
...
## Object Indirection (Объектное посредничество)
...
> "любое взаимодействие с данными, поведением или модулями, должно быть реализовано не напрямую, а через промежуточный объект-посредник."

Как говорится найди 5 отличий от:

**Interface Segregation** (разделение интерфейсов) — взаимодействие реализуется не напрямую, а через интерфейс.

### Суть принципа

Цель посредничества — снизить зацепление между компонентами системы. Вместо прямого вызова методов одного модуля другим, мы вводим промежуточный слой, который берёт на себя ответственность за организацию взаимодействия

Можно рассматривать принцип **Indirection** как конкретное средство достижения **Low Coupling**.

### Историческая аналогия

Можно думать о посреднике как о [телефонных барышнях](https://telhistory.ru/telephone_history/interesnye-fakty/phone-ladies/). Представьте телефонную связь начала XX века. В квартире поэта А. А. Блока стоял телефон без диска и кнопок. Чтобы позвонить, абонент вращал ручку индуктора, который давал напряжение около 60 вольт. Оно шло по проводам телефонной линии на коммутатор. При этом на коммутаторе, за которым сидела телефонистка, автоматически открывался бленкер. Надо было сказать примерно следующее: "Барышня, Солянка, два-семнадцать". Это означало, что девушке нужно было воткнуть штекер на другом конце шнура в семнадцатое гнездо второго ряда на панели, к которой были подсоединены аппараты района Солянки. Девушка соединяла абонентов или обращалась к соседке, которая обслуживала район, где находился требуемый номер

Это классический пример посредничества: абоненты не могли поговорить друг с другом напрямую, взаимодействие шло через оператора.
...
Вместо определения или хоть каких-то слайдов в презентации Великий Георгий Круглов Сказал: "Лол, используйте полиморфизм"

Немного расширим этот "Лол":

> «Полиформизм - способность объектов разных классов рассматриваться как объекты общего суперкласса, что позволяет использовать один и тот же интерфейс или метод для разных типов данных. Полиформизм позволяет одной функции, методу или интерфейсу работать с объектами разных классов, где каждый класс может иметь свою собственную реализацию этого интерфейса»

Такая концепция повышает переиспользуемость кода, гибкость и расширяемость, позволяя методам вести себя по-разному в зависимости от конкретного объекта, который их вызывает, а также позволяет разработчикам писать более универсальный и модульный код, который может работать с новыми классами без изменений в существующем коде.
...
Можно сказать, что полиформизм в этом контексте означает «давать одно и то же имя операциям в разных объектах». Хотя это и не есть полное определение.
...
> «Скрытие нестабильных деталей за стабильным интерфейсом»
...
> Введение искусственного вспомогательного класса, который не отражает предметную область»
...
1. **Low Coupling & High Cohesion** — стремитесь к слабым связям между модулями и сильной связности внутри них
2. **Information Expert** — назначайте ответственность тому, кто владеет данными
3. **Creator** — создавать объекты должен тот, кто ими владеет или использует
4ы. **Controller** — организуйте точку входа для координации системных операций
...
1. Можно ли снизить зависимости между классами? → **Low Coupling** + **Indirection**
2. Кто должен знать эту информацию? → **Information Expert**
3. Кто должен создать этот объект? → **Creator**
4. Кто обработает это системное событие? → **Controller**
