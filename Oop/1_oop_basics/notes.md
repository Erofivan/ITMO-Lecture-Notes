...
2. [Основные концепции ООП](#основные-концепции-ооп)
    1. [Абстракция](#1-абстракция-abstraction)
    2. [Инкапсуляция](#2-инкапсуляция-encapsulation)
    3. [Сокрытие](#3-сокрытие-information-hiding)
    4. [Наследование](#4-наследование-inheritance)
    5. [Полиморфизм](#5-полиморфизм-polymorphism)
    6. [Композиция](#6-композиция-composition)
    7. [Ассоциация](#7-ассоциация-association)
    8. [Агрегация](#8-агрегация-aggregation)
...
{
    // Эти два поля являются композицией 
    public int _balance { get; private set };
    public string _name { get; private set };

    Account(string name)
    {
        _name = name; // Поле _name получается через агрегацию
        _balance = GetBalance(name); // Поле _balance получается через ассоциацию
    }
}
```
...
Ещё одним достаточно попрулярным способ обращения является `TryGetXxx` конвенция:
```csharp
...
Он пытается получить значение и если у него не получается, то, в отличие от простого Get, вместо того, чтобы выбросить исключение мы возвращаем false или же какой-то Result Type (подробнее об этом в следующем пункте), а в out записываем null. Если значение найдено, то мы возвраем true и само значение. 

Обратите внимание, что в отличие от `TryGetXxx` `TryFindXxx` практически не используется. Почему? Потому что это обычно бессмысленно. В случае использования `Find` обычно null не является каким-то исключением и его можно спокойно обработать. То есть используется очень редко и в тех случаях когда мы хотим сделать получение null ошибкой.
...
| `TryGetXxx` | `bool`/`Result Type` + `out T?` | Возвращает `false` | Альтернатива `Find` (стиль .NET) |
...
switch (result)
{
    ...
    case AddStudentResult.AlreadyMember:
        Console.WriteLine(" Студент уже состоит в группе");
        break;
...
Console.WriteLine("Неожиданный результат");
```
...
**Примеры**:
- Вместо `decimal` используем `Money`
- Вместо `string` используем `Email`
- Вместо `double` используем `Distance`
...
Типовой пример ошибки делать что-то вроде такого:
```csharp
while (x < money.Value)  // Прямое обращение к внутренним данным
{
    x += 100;
}
```
Это совершенно бесполезно. Зачем создавать VO, если мы потом нарушаем инкапсулцию и обращаемся напрямую через .Value? 

Ещё один яркий пример нарушения инкапсуляции:
```csharp
public class BankAccount
{
    public Money Balance { get; private set; }
    
    public WithdrawResult TryWithdraw(Money amount)
    {
        // Проверяем преед вычитанием
        if (Balance.Value < amount.Value) // Чистейшее нарушение инкапсуляции. Почему бы просто не перегрузить оператор сравнения?
...
#### Операторы и сравнение между VO и не VO

Ещё одной типовой ошибкой является перегрузка операторов между VO и не VO:

```csharp
// Сделали перегрузку для Money и int
public static Money operator +(Money left, int right)
{
    return new Money(left.Value + right);
}

// Сравнение по значению Money и int
...
Это ошибка. Как мы можем сравнить объект (Money) и просто какое-то число. Это тоже самое, что в физике сравнивать числа разной разнмерности (например, сравнить скорость с напряжением - не имеет никакого смысла) или как в детских задачах: летели два верблюда: один рыжый - другой налево, сколько весит кусок асфальта, если ёжику 24 года? То есть просто несравнимные бессмысленные значения

#### Протёкшая абстракция

Представьте, что у нас есть Value Object `Money`, который не может быть отрицательным. Любая попытка созадть что-то вроде `new Money(-100)` выбросит исключение. Где это может привести к ошибкам:
```csharp
public static Money operator -(Money left, Money right)
{
    // Если right > left, то мы попытаемся создать отрицательный Money,
...
public class Account
{
    ...
    public void Withdraw(Money amount)
    {
        // Если мы теперь попытаемся вычесть, то оператор вычитания окажется опасен
        Balance = Balance - amount; 
    }
}
```

Пользователь не должен лезть в реализации и пытаться понять может ли у нас тот или иной оператор кинуть исключение. Тем более сигнатура об этом не говорит. 

Соответсвенно при разрабокте мы должны заранее подумать о том, может ли у нас быть выброшено исключение от того или иного метода и если да, то мы можем применить TryXxx паттерн:
...
public readonly record struct Money 
{
    ...
    // Мы не определеяем operator -
    // public static Money operator -(Money left, Money right) => ...

    // Вместо этого мы делаем так:
    public static bool TrySubtract(Money left, Money right, out Money result)
    {
        decimal newAmount = left.Amount - right.Amount;
        if (newAmount < 0)
        {
            result = default;
            return false;
        ...
        result = new Money(newAmount); // Мы уверены, что конструктор не кинет exception
        return true;
    }
}
```

Таким образом мы можем использовать что-то такое в вызывающем коде:
...
public class Account
{
    ...
    public AccountResult Withdraw(Money amount)
    {
        if (Money.TrySubtract(Balance, amount, out Money newBalance))
        {
            Balance = newBalance
        }
        else
        {
            return new AccountResult.InsufficientFundsException();
        }
    }
}
```

Конечно, мы могли бы каждый раз проверять оператор вычитания:
...
public class Account
{
    ...
    public AccountResult Withdraw(Money amount)
    {
        // 1. Сначала проверяем (Guard Clause)
        if (Balance < amount)
        {
            return new AccountResult.InsufficientFundsException();
        }

        // 2. Теперь мы уверены, что вычитание безопасно
        // Этот код уже никогда не кинет ArgumentException
        Balance = Balance - amount; 
    }
}
```

Но такой подход не рекомендуется: мы легко можем забыть проверку. Да и опять-таки: тут есть немного протёкшей абстракции. 

#### Создание объектов и сложные операции

Представьте, что мы моделируем какую-нибудь физическую модель, например, делаем виртуальную лабораторию. У нас есть разные Value Objects вроде `Acceleration`, `Force`, `Mass`, `Speed` и так далее

Допустим, мы хотим посчитать ускорение, используя силу и массу. Любому второкласснику известно, что `A = F / m`, соответственно, мы можем попытаться перегрузить оператор деления:
...
public readonly struct Acceleration
{
    ...
    // Предположим, что mass всегда больше 0 (>0) и исключения не будет
    // A даже если будет, то это уже ответсвенность вызываемого класса
    // ведь это обычное деление. Тоже самое, что int / int - здесь ошибка 
    // при делении на ноль ожидаема
    public static Acceleration operator/(Force force, Mass mass) => new Acceleration(force.Value / mass.Value);
}
```

Это может даже показаться очень красивым решением: мы идеально перегрузили все операторы (умножение, деление, сложение и так далее) для наших формул и теперь можем спокойно с ними работать. Но мы кое-что забываем: люди могут не знать физику. Они могут попытаться сделать `A = F * M`, `A = F^M`, `A = F-M` и так далее. 

Это сложная логика (даже если кому-то она кажется простой). Сложных операторов быть не должно. Типичный пример сложного оператора - это создание VO из других VO (как в примере выше) или, иногда, деление, умножение или ещё что-то. То есть нам всегда лучше оставаться внутри своего домена. 

В таких случаях вместо операторов применяют Create/TryCreate методы:
```csharp
public readonly struct Acceleration
{
    public Acceleration(double value)
...
То есть мы как бы "создаём" ускорение из силы и массы. Нам не нужно думать, что это за формула. Нам не нужно ничего знать. Мы просто значем, что мы можем получить ускорение из силы и ускорение, а вот как - это уже заботы метода. На это можно смотреть так: "создай как-то то, что нам нужно из того, что мы дали". 

Ещё можно использовать TryCreate паттерн
```csharp
public readonly struct Acceleration
{
    public Acceleration(double value)
    ...
    public static bool TryCreate(Force force, Mass mass, out Acceleration acceleration)
    {
        // Обратите внимание, что здесь, в отличие от примеров выше, мы допускаем нулевую массу
        if (mass == 0) 
        {
        ...
        acceleration = new Acceleration(force.Value / mass.Value);
        return true;
    }
...
Такой метод можно использовать, чтобы проверять сумели ли мы создать и потом это как-то обрабатывать (по аналогии с `TrySubstract` в примере выше)

Когда выбирать Create, а когда TryCreate? Если мы понимаем, что у нас не может быть исключений в объекте (масса не может быть равна нулю) или же этих исключений будет очень мало и мы эти исключения хотим, то мы можем использовать просто Create - просто для того, чтобы создать объект. Если же мы можем получить исключение (масса равна нулю, а значит можем поделить на ноль) или же у нас будут очень часто возникать исключения вместо контролируемых результатов, то лучше использовать TryCreate - "попытайся что-то создать".
