...
**SOLID** — это акроним, образованный от названий пяти фундаментальных принципов объектно-ориентированного проектирования, сформулированных Робертом Мартином (Robert C. Martin, также известным как "Uncle Bob") в начале 2000-х годов:
...
Когда в вашем приложении всего 200 строк, то дизайн как таковой вообще не нужен. Достаточно аккуратно написать 5-7 методов и всё будет хорошо. Проблемы возникают, когда система растёт и увеличивается в масштабах. Когда класс разрастается, он просто перестаёт помещаться в голове. 
...
Каждый метод, класс или модуль должен выполнять только одну конкретную задачу. Если компонент отвечает за несколько операций сразу, то внося изменения, касающиеся одной из операций вы, сами того не подозревая, можете затронуть и другие. Поэтому их следует разделить на отдельные компоненты.
...
То есть идея в том, что мы разбиваем то, что делает много всего и сразу, на несколько составных частей, каждая из которых отвечают за что-то одно.
...
// АНТИПАТТЕРН: Этот класс знает сликшом
...
1. **Очень маленькие приложения** — избыточная абстракция усложнит понимание
...
**Правило применения:** Применяйте SRP осознанно, когда видите, что класс имеет несколько причин для изменения или делает несколько несвязанных вещей. Не стремитесь к абсолютной декомпозиции в ущерб простоте. Иначе это уже будет ошибкой излишней декомпозиции
...
**Open/Closed Principle (OCP)** — принцип открытости/закрытости — гласит, что *проектировать типы необходимо, таким образом чтобы их логику можно было расширять, не изменяя их исходный код*, то есть *типы должны быть открыты для расширения, но закрыты для изменения*.

Другими словами: **расширяйте поведение системы, не изменяя уже существующий код**.
...
5. **Система гарантировано будет расширяться редко** например синтаксис языка программирования
...
**Liskov Substitution Principle (LSP)** — принцип подстановки Барбары Лисков — гласит, что *проектировать иерархию типов необходимо таким образом, чтобы логика дочерних типов не нарушала инвариант и интерфейс родительских типов*.

Условно: объекты подклассов должны быть взаимозаменяемы с объектами базового класса без нарушения корректности программы или же, что должна быть возможность вместо базового типа подставить любой его подтип.
...
TestRectangleArea(square); // Ошбика: площадь = 100, а не 50
...
// Такие проверки — признак нарушения LSP и OCP!
...
{
    // Ошбика: Более конкретный тип (Cat вместо Animal)
...
{
    // Ошбика: Возвращаем более общий тип
...
{
    // Ошбика: выбрасывает новый тип исключения
...
{
    // Ошбика: Усиливаем предусловие (добавляем money <= 100)
...
class MicroAccount : Account
{
    // Ошбика: Не выполняем постусловие (не добавляем бонус)
...
class MicroAccount : Account
{
    ...
    // Ошбика: Нарушаем инвариант родителя
...
**Interface Segregation Principle (ISP)** — принцип разделения интерфейсов — гласит, что *проектировать нужно маленькие абстракции, которые ответственны за свой конкретный функционал, а не одну всеобъемлющею, содержащую много различного*.

Другими словами: *лучше много узких специализированных интерфейсов, чем один универсальный "бог-интерфейс"* или же *клиенты не должны вынуждено зависеть от методов интерфейса, которые они не используют*
...
// Принтеру не нужно играть музыку и печь хлеб
// P.S. Хотя я бы от такого принтера не отказался...
// Это правада удобно - не нужна колонка, так как 
// он обычно в комнате и на месте куда можно поставить
// колонку + каждое утро свежий хлеб 
...
// В отличии от множественного наследования классов
// C# поддерживает множественное наследование интерфейсов
...
**Dependency Inversion Principle (DIP)** — принцип инверсии зависимостей — гласит, что *проектировать типы необходимо таким образом что одни реализации не зависят от других напрямую*.


Другими словами: 
1. *Модули верхнего уровня не должны зависеть от модулей нижнего уровня. Оба должны зависеть от абстракций.*
2. *Абстракции не должны зависеть от деталей. Детали должны зависеть от абстракций.* или же: *зависьте от интерфейсов, а не от конкретных реализаций*.
...
```csharp
string content = "Это текст конспекта о принципах SOLID";
...
Вот так может выглядеть архитектура программы после применения принципов SOLID:
