...
**Liskov Substitution Principle** (LSP, Принцип подстановки Барбары Лисков) — третий принцип SOLID, сформулированный Барбарой Лисков в 1987 году. Он гласит:
...
Другими словами: **объекты подклассов должны вести себя так же, как объекты родительского класса. Подклассы должны дополнять, а не замещать поведение базового класса**.

### Суть принципа

Если у вас есть класс и вы создаёте на его основе подкласс, то:

- **Подкласс должен выполнять те же операции**, что и родитель
- **Подкласс должен возвращать те же типы результатов**
- **Подкласс должен соблюдать контракты** (предусловия, постусловия, инварианты) родителя
- **Код, работающий с родителем, должен корректно работать с подклассом** без изменений

**Простая проверка:** Если вы можете заменить объект родительского класса объектом подкласса, и программа продолжит работать корректно — LSP соблюдён.

### Визуальная иллюстрация

![Кофе и напитки — иерархия, соблюдающая LSP](src/coffee.png)

На диаграмме видно, что все подклассы `Beverage` (напитки) сохраняют поведение родителя — их можно подавать и пить. Это корректная иерархия.

### Классический пример: Прямоугольник и Квадрат

Этот пример демонстрирует, как интуитивно правильная иерархия наследования может нарушать LSP.

#### Нарушение LSP

Математически квадрат — это частный случай прямоугольника (прямоугольник с равными сторонами). Попробуем выразить это в коде:

```csharp
// ❌ ПЛОХО: Нарушение LSP через неправильное наследование
class Rectangle
{
...
{
    // При изменении ширины автоматически меняем высоту
    public override int Width
    {
        ...
        {
            base.Width = value;
            base.Height = value;  // Поддерживаем инвариант квадрата
        }
    }

    // При изменении высоты автоматически меняем ширину
    public override int Height
    {
        ...
        {
            base.Height = value;
            base.Width = value;  // Поддерживаем инвариант квадрата
        }
    }
}
```

**Проверим, нарушен ли LSP:**
...
class Program
{
    ...
    {
        // Код ожидает Rectangle, но получает Square
        Rectangle rect = new Square();
        TestRectangleArea(rect);  // 💥 Выбросит исключение!
    }

    public static void TestRectangleArea(Rectangle rect)
    {
        // Для обычного прямоугольника это корректное поведение
        rect.Height = 5;
        rect.Width = 10;
...
1. Устанавливаем `Height = 5` → у `Square` оба измерения становятся 5 (5×5)
2. Устанавливаем `Width = 10` → у `Square` оба измерения становятся 10 (10×10)
3. Площадь = 10 × 10 = **100**, а не 50!
4. Программа выбрасывает исключение

**Проблема:** `Square` ведёт себя **не так, как `Rectangle`**. Код, который корректно работает с прямоугольником, ломается с квадратом. Это классическое нарушение LSP.

#### Плохое решение: Проверка типов

Некоторые пытаются "обойти" проблему проверкой типов:

```csharp
// ❌ АНТИПАТТЕРН: Проверка типов вместо правильной архитектуры
public static void TestRectangleArea(Rectangle rect)
{
    if (rect is Square)
    {
        // Особая логика для квадрата
        rect.Height = 5;
        if (rect.GetArea() != 25)
...
**Почему это плохо:**
- Нарушается полиморфизм — код знает о конкретных типах
- Нарушается OCP — при добавлении нового типа придётся изменять метод
- Признак того, что архитектура классов неправильная

#### Правильное решение

Проблема в том, что **квадрат и прямоугольник не должны быть в отношении наследования**. Они слишком разные в своём поведении.

**Решение 1: Общий интерфейс без наследования**

```csharp
// ✅ ХОРОШО: Общий интерфейс вместо наследования
interface IShape
{
    int GetArea();
}
...
**Решение 2: Использование базового класса для общей логики**

```csharp
// ✅ ХОРОШО: Базовый класс с защищёнными полями
...
```csharp
// Теперь код работает с абстракцией IShape
...
PrintArea(rectangle);  // Площадь: 50
PrintArea(square);     // Площадь: 25
```

**Ключевой вывод:** Производный класс, который может делать **меньше**, чем базовый, обычно нельзя безопасно подставить вместо базового. Это нарушает LSP.

### Пример: Птицы и пингвины

Ещё один классический пример нарушения LSP — иерархия животных, где не все подклассы могут выполнять действия базового класса.

#### Нарушение LSP: Версия 1

```csharp
// ❌ ПЛОХО: Penguin не может выполнить FlyTo(), нарушая контракт Bird
...
{
    // Контракт: все птицы умеют летать
    public virtual void FlyTo(Coordinate coordinate)
    {
        Console.WriteLine($"Flying to ({coordinate.X}, {coordinate.Y})");
    }
}

// Пингвин — птица, но не умеет летать!
public class Penguin : Bird
{
    // Вместо полёта — смерть. Полностью меняет контракт!
    public override void FlyTo(Coordinate coordinate)
    {
        Die();  // 💥 Нарушение ожиданий
    }
}
```

**Использование, которое демонстрирует проблему:**

```csharp
void StartMigration(IEnumerable<Bird> birds, Coordinate destination)
{
    foreach (var bird in birds)
    {
        // Ожидаем, что bird полетит к destination
        bird.FlyTo(destination);
    }
}

// Использование
var birds = new Bird[] { new Penguin(), new Sparrow() };
StartMigration(birds, new Coordinate(100, 200));
// 💥 Penguin умирает вместо того, чтобы лететь!
```

**Проблема:** Функция `StartMigration` работает с типом `Bird` и ожидает, что все птицы умеют летать. Но `Penguin` нарушает это ожидание — вместо полёта он "умирает". Это классическое нарушение LSP.

#### Нарушение LSP: Версия 2 (с проверкой типов)

Попытка обойти проблему через проверку типов:

```csharp
// ❌ АНТИПАТТЕРН: Type checking вместо правильной архитектуры
public class Bat : Creature
{
    public void FlyTo(Coordinate coordinate)
    {
        Console.WriteLine($"Bat flying to ({coordinate.X}, {coordinate.Y})");
    }
}

void StartMigration(IEnumerable<Creature> creatures, Coordinate destination)
{
    foreach (var creature in creatures)
    {
        // Проверяем конкретные типы — плохой код!
        if (creature is Bird bird)
        {
            bird.FlyTo(destination);
        }
        else if (creature is Bat bat)
        {
            bat.FlyTo(destination);
        }
        // Каждый новый летающий вид требует изменения этого метода
    }
}
```

**Почему это плохо:**
- Нарушается полиморфизм
- При добавлении нового летающего существа (например, `Butterfly`) придётся изменять `StartMigration`
- Нарушается OCP — метод не закрыт для изменений

#### Правильное решение: Разделение по способностям

```csharp
// ✅ ХОРОШО: Интерфейсы определяют способности
public record Coordinate(int X, int Y);
...
// Птица — базовый класс для всех птиц
public class Bird : CreatureBase { }

// Пингвин — птица, которая НЕ умеет летать
public class Penguin : Bird 
{
    // Не реализует IFlyingCreature — честно признаёт, что не летает
}

// Колибри — птица, которая умеет летать
public class Colibri : Bird, IFlyingCreature
{
    public void FlyTo(Coordinate coordinate)
    {
        Console.WriteLine($"Colibri flying to ({coordinate.X}, {coordinate.Y})");
    }
}

// Летучая мышь — не птица, но умеет летать
...
```csharp
void StartMigration(IEnumerable<IFlyingCreature> creatures, Coordinate destination)
{
    foreach (var creature in creatures)
    {
        // Симулируем миграцию с риском
        if (Random.Shared.NextDouble() < 0.8)
        {
            creature.FlyTo(destination);  // 80% долетают
        }
        else
        {
            creature.Die();  // 20% погибают в пути
        }
    }
}
...
StartMigration(flyingCreatures, new Coordinate(420, 69));
// ✅ Penguin не попадёт в миграцию, так как не реализует IFlyingCreature
```

**Преимущества правильной архитектуры:**
- `Penguin` честно не реализует `IFlyingCreature` — нет ложных обещаний
- Метод `StartMigration` принимает только летающих существ
- Легко добавлять новых летающих существ без изменения кода
- LSP соблюдён — все `IFlyingCreature` гарантированно умеют летать
