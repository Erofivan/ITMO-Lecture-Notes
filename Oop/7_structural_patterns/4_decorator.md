...
> Тип-обёртка над объектом абстракции, которую он реализует добавляет к поведениям объекта новую логику
...
**Abstraction/Component** (абстракция/компонент) — общий интерфейс или абстрактный класс, который определяет базовое поведение объектов, которые могут быть обёрнуты в декораторы.
