...
Здравствуйте. Сегодня мы подробно разберём один из фундаментальных порождающих паттернов — Фабричный метод. Наша задача — понять не просто как он устроен, а зачем он нужен, какую проблему он решает, и как он это делает, используя ключевые принципы ООП: наследование и полиморфизм для достижения вариативности создания объектов.

Давайте следовать логике лекции, шаг за шагом.

1. Контекст: Проблема
Давайте начнём с примера. Представьте, что мы пишем систему обработки заказов.

(Слайд "пример использования фабричный метод" - первый код)
...
// ... (Order, OrderItem)
Здесь у нас есть класс PaymentCalculator. Его задача — взять заказ (Order), посчитать итоговую стоимость (totalCost), применить какие-то скидки (это бизнес-логика) и... вот она, ключевая строка: return new CashPayment(totalCost);.

В чём здесь проблема? Наш PaymentCalculator жёстко привязан к конкретному типу оплаты — CashPayment (оплата наличными). Вся его сложная бизнес-логика по расчёту скидок и купонов теперь неразрывно связана с тем, что в конце мы создаём именно CashPayment.
