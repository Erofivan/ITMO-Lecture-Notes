...
> **Ключевые выводы по Фабричному методу:**
> - Делегирует создание объектов подклассам через полиморфизм
> - Основная логика в базовом классе, создание - в подклассах
> - Соблюдает принцип открытости/закрытости (OCP)
> - Идеален для расширяемых систем с неизвестными заранее типами


💡 **Практический совет:**
В современном C# часто можно избежать Фабричного метода, используя dependency injection (DI) контейнеры. 

Они предоставляют более гибкий механизм создания и управления зависимостями.


💡 **Практический совет:**
В ASP.NET Core вместо ручной реализации Singleton используйте встроенную регистрацию служб:
...
Это обеспечит потокобезопасность и правильное управление временем жизни.
...
> **Ключевые выводы по Строителю:**
> - Пошагово создаёт сложные объекты
> - Отделяет конструирование от представления
> - Fluent Interface делает код читаемым
> - Два варианта: Convenience Builder и Stateful Constructor
...
> **Ключевые выводы по Абстрактной фабрике:**
> - Создаёт семейства связанных объектов
> - Гарантирует совместимость продуктов в семействе
> - Использует композицию вместо наследования
> - Сложнее добавить новый тип продукта, проще - новое семейство
...
### Упражнение 1: Фабричный метод
Реализуйте систему уведомлений (email, SMS, push) используя Фабричный метод.
Создайте базовый класс `NotificationSender` с методом `Send()` и фабричным методом `CreateNotification()`.

### Упражнение 2: Абстрактная фабрика
Создайте систему UI компонентов для двух тем (Light и Dark).
Каждая тема должна содержать кнопки, поля ввода и чекбоксы.

### Упражнение 3: Строитель
Реализуйте строитель для создания HTTP запросов с различными параметрами:
метод, URL, headers, body, timeout и т.д.

### Упражнение 4: Прототип
Создайте систему конфигурационных шаблонов для развёртывания приложений
с возможностью клонирования базовых конфигураций.

### Упражнение 5: Одиночка
Реализуйте логгер приложения, используя Singleton с поддержкой различных
уровней логирования (Debug, Info, Warning, Error).

**Совет:** Начните с простой реализации, затем постепенно добавляйте функциональность.
Не забывайте про unit-тесты!
...
|-------------------|-----------------|----------|
| Creational Patterns | Порождающие паттерны | Паттерны, решающие задачи создания объектов |
...
| Builder | Строитель | Пошагово создаёт сложные объекты |
| Prototype | Прототип | Создаёт объекты путём клонирования |
| Singleton | Одиночка | Гарантирует единственность экземпляра |
| Product | Продукт | Создаваемый объект |
| Creator | Создатель | Класс, создающий продукты |
| Fluent Interface | Текучий интерфейс | Стиль API с цепочкой вызовов |
| Deep Copy | Глубокое копирование | Копирование со всеми вложенными объектами |
| Shallow Copy | Поверхностное копирование | Копирование только ссылок |
| Lazy Initialization | Ленивая инициализация | Создание объекта при первом обращении |
| Thread-Safety | Потокобезопасность | Корректная работа в многопоточной среде |
...
**Быстрые ссылки:**
- [К началу](#порождающие-паттерны-creational-patterns)
...
- [Сравнение паттернов](#сравнение-порождающих-паттернов)

---

**Статус документа:** ✅ Полностью отрефакторен (ноябрь 2025)

**Версия:** 2.0

**Язык:** Русский (с английскими терминами)

**Примеры кода:** C# (.NET 9.0+)

**Лицензия:** Материалы для образовательных целей ИТМО
