# Порождающие паттерны (Creational Design Patterns)
...
В процессе разработки программного обеспечения мы постоянно сталкиваемся с задачей создания объектов. Казалось бы, что может быть проще — используй оператор `new` и готово. Однако в реальных приложениях создание объектов часто становится сложной задачей:

- Объект может требовать сложной инициализации с множеством параметров
- Логика создания может зависеть от контекста или конфигурации
- Необходимо создавать целые семейства связанных объектов
- Требуется контролировать количество экземпляров класса
- Нужно обеспечить гибкость и расширяемость при добавлении новых типов
...
1. **Дублирование кода**: Логика создания заказа повторяется в разных местах
2. **Жёсткая связанность**: Код напрямую зависит от конкретных классов
3. **Сложность изменений**: При изменении логики создания придётся править код везде
4. **Отсутствие контроля**: Нельзя ограничить или отследить процесс создания
...
- **Factory Method (Фабричный метод)**: Определяет интерфейс для создания объекта, но оставляет подклассам решение о том, какой класс инстанцировать. Использует наследование и полиморфизм.

- **Abstract Factory (Абстрактная фабрика)**: Предоставляет интерфейс для создания семейств связанных или зависимых объектов без указания их конкретных классов. Использует композицию.

- **Builder (Строитель)**: Отделяет конструирование сложного объекта от его представления, позволяя одному и тому же процессу создавать различные представления.

- **Prototype (Прототип)**: Определяет виды создаваемых объектов с помощью экземпляра-прототипа и создаёт новые объекты путём копирования этого прототипа.
...
Со временем программа получает популярность, и морские перевозчики обращаются с просьбой добавить поддержку морской логистики. Это прекрасная возможность для расширения бизнеса, однако возникает техническая проблема: существующий код жёстко привязан к классу `Truck`, и интеграция нового класса `Ship` (судно) потребует значительных изменений по всей кодовой базе.

![](src/factory_method/factory_method_proble.png)

Добавление класса `Ship` в существующую систему потребует модификации значительной части кода. Более того, если в будущем потребуется поддержка авиаперевозок или железнодорожного транспорта, весь процесс придётся повторить заново.
...
На первый взгляд может показаться, что мы просто переместили вызов конструктора из одного места в другое, не получив реальной пользы. Однако ключевое преимущество проявляется при использовании наследования: теперь мы можем переопределить фабричный метод в подклассе, изменив тип создаваемого объекта без изменения остального кода.

**Важное условие**: для корректной работы паттерна все создаваемые объекты должны реализовывать общий интерфейс. Это позволит клиентскому коду работать с продуктами через абстракцию, не завися от конкретных классов.

![](src/factory_method/factory_method_interface.png)

Теперь можно переопределить фабричный метод в подклассе, изменив тип создаваемого продукта без модификации базовой бизнес-логики.

Рассмотрим пример: классы `Truck` (грузовик) и `Ship` (судно) реализуют интерфейс `ITransport` с методом `Deliver()` (доставить). Каждый класс реализует доставку по-своему: грузовики перевозят груз по земле, суда — по морю. Фабричный метод в классе `RoadLogistics` (дорожная логистика) возвращает объект `Truck`, а класс `SeaLogistics` (морская логистика) — объект `Ship`.
...
1. **Product (Продукт)** — определяет общий интерфейс объектов, которые могут быть созданы фабричным методом. Этот интерфейс должны реализовывать все конкретные продукты.

2. **Concrete Product (Конкретный продукт)** — содержит различные реализации интерфейса Product. Каждый конкретный продукт предоставляет свою специфическую реализацию методов интерфейса.

3. **Creator (Создатель)** — объявляет фабричный метод, возвращающий объект типа Product. Важно, что тип возвращаемого значения должен соответствовать интерфейсу Product, а не конкретному классу.

Фабричный метод часто объявляется абстрактным (`abstract`), заставляя все подклассы предоставить собственную реализацию. Однако он может содержать и реализацию по умолчанию, возвращающую стандартный продукт.

Следует понимать, что создание объектов — не единственная функция класса Creator. Обычно он также содержит основную бизнес-логику работы с продуктами. Аналогия: крупная IT-компания может иметь учебный центр для подготовки разработчиков, но основная задача компании — создание программных продуктов, а не обучение персонала.

4. **Concrete Creator (Конкретный создатель)** — переопределяет фабричный метод, создавая и возвращая конкретный тип продукта.
...
В этом примере Factory Method помогает создавать кросс-платформенные элементы пользовательского интерфейса, изолируя основной код от конкретных классов UI-компонентов.

Фабричный метод объявлен в базовом классе `Dialog` (диалоговое окно). Его подклассы соответствуют различным операционным системам или платформам. Благодаря фабричному методу, логику диалоговых окон не нужно переписывать для каждой платформы — подклассы наследуют общую функциональность, изменяя только типы создаваемых UI-элементов.

Базовый класс `Dialog` работает с кнопками через общий интерфейс `IButton`. Независимо от того, какую конкретную реализацию кнопки возвращает фабричный метод (Windows, Web или macOS), диалоговое окно будет корректно функционировать. Базовый класс не зависит от конкретных реализаций, делегируя выбор подклассам.

Этот подход можно расширить и на другие элементы интерфейса (текстовые поля, чекбоксы, списки). Однако при необходимости создания множества связанных элементов одновременно имеет смысл рассмотреть паттерн Abstract Factory.

```csharp
// ============================================
// Пример: Кросс-платформенные UI элементы
// Демонстрирует применение Factory Method
// для создания платформо-специфичных компонентов
// ============================================

using System;

// ============================================
// Шаг 1: Определяем интерфейс Product
// ============================================

/// <summary>
/// IButton - общий интерфейс для всех кнопок (Product)
/// Все конкретные реализации кнопок должны реализовать этот интерфейс
/// </summary>
...
// ============================================
// Шаг 2: Создаём конкретные продукты (Concrete Products)
// ============================================

/// <summary>
/// WindowsButton - конкретная реализация кнопки для Windows (Concrete Product A)
/// Реализует отрисовку и обработку событий в стиле Windows
/// </summary>
...
/// <summary>
/// HtmlButton - конкретная реализация кнопки для Web (Concrete Product B)
/// Генерирует HTML-разметку и использует JavaScript для обработки событий
/// </summary>
...
// ============================================
// Шаг 3: Создаём Creator с фабричным методом
// ============================================

/// <summary>
/// Dialog - абстрактный создатель (Creator)
/// Содержит бизнес-логику работы с диалоговыми окнами
/// Не зависит от конкретных типов кнопок
/// </summary>
...
// ============================================
// Шаг 4: Создаём Concrete Creators
// ============================================

/// <summary>
/// WindowsDialog - конкретный создатель для Windows (Concrete Creator A)
/// Переопределяет фабричный метод для создания Windows-кнопок
/// </summary>
...
/// <summary>
/// WebDialog - конкретный создатель для Web (Concrete Creator B)
/// Переопределяет фабричный метод для создания HTML-кнопок
/// </summary>
...
// ============================================
// Шаг 5: Клиентский код (Application)
// ============================================

/// <summary>
/// Application - главный класс приложения
/// Демонстрирует использование Factory Method
/// </summary>
...
--- Сценарий 1: Windows ---
[Application] Инициализация для платформы: windows
[WindowsDialog] Начало отрисовки диалога...
[Windows] Привязан обработчик события WM_LBUTTONDOWN
[Windows] Отрисована нативная кнопка в стиле Windows 11
[WindowsDialog] Диалог отрисован успешно

--- Сценарий 2: Web ---
[Application] Инициализация для платформы: web
[WebDialog] Начало отрисовки диалога...
[HTML] Привязан обработчик через addEventListener
[HTML] Сгенерирована разметка: <button class="btn">OK</button>
[WebDialog] Диалог отрисован успешно
...
### Структура паттерна Factory Method

Паттерн состоит из двух фундаментальных компонентов:

**Product (Продукт)** — абстрактный тип создаваемых объектов. Определяет общий интерфейс для всех конкретных реализаций. Обеспечивает полиморфизм, позволяя клиентскому коду работать с любым продуктом через единый интерфейс.

**Creator (Создатель)** — абстрактный класс, содержащий основную бизнес-логику и объявляющий фабричный метод. Подклассы Creator'а реализуют фабричный метод, создавая конкретные типы продуктов.

**Ключевая идея**: Creator делегирует создание конкретного экземпляра Product подклассам. Базовый класс определяет *когда* создавать, подклассы — *что* создавать.

![](src/factory_method/factory_method_scheme.png)

### Практический пример: Система обработки платежей

Рассмотрим задачу из области электронной коммерции.
...
В чём здесь проблема? Наш `PaymentCalculator` привязан к конкретному типу оплаты — `CashPayment`. Фактически мы применяем какие-то абстрактные купоны и скидки, а потом всё равно возвращаем оплату наличными `CashPayment` 

Если завтра бизнес скажет: "Мы хотим добавить оплату банковским переводом"? Или криптовалютой? Нам придётся:
...
Иными словами `Creator` делегирует создание product'а своим наследникам. Метод, который он делегирует, и называется Фабричный метод

Рассмотрим, как наш код преображается с применением этого паттерна:

```csharp
// Определяем интерфейс для продукта
// Общий интерфейс для всех типов платежей
...
Мы не тронули ни одной строки существующего кода (PaymentCalculator, CashPaymentCalculator и так далее.). Мы только добавили новый. Система расширяется, оставаясь "закрытой" для модификаций. Это принцип Открытости/Закрытости (Open/Closed Principle) в чистом виде.
...
Представьте, что у нас появляется другая иерархия калькуляторов, например, FixedPricePaymentCalculator, у которого другая логика Calculate (например, он всегда возвращает 100). Иными словами он не применяет скидки и купоны и не делает никакой другой бизнес-логики, в отличии от обычного PaymentCalculator.
...
**3. Неявное нарушение SRP:**: объект конкретного создателя ответственен как за реализацию логики, так и за создание продуктов.
...
Вывод:
Фабричный метод (Factory Method) — это классический паттерн, который решает проблему связи с оператором new путём делегирования создания наследникам через абстрактный метод. Он идеально подходит, когда у вас есть общая логика, но в конце этой логики нужно создавать разные объекты, тип которых определяется подклассом.
...
1. **Именование**: Используйте глаголы `Create`, `Make`, `Build` для фабричных методов
2. **Возвращаемый тип**: Всегда возвращайте интерфейс, а не конкретный класс
3. **Параметры**: Минимизируйте количество параметров фабричного метода
4. **Валидация**: Выполняйте проверку параметров в фабрике, а не в конструкторе
5. **Документация**: Чётко документируйте, какие типы возвращает каждая фабрика



**Недостатки:**
- ❌ Может привести к созданию больших параллельных иерархий классов
- ❌ Сильная связанность с базовым классом через наследование

**Когда использовать:**
- Заранее неизвестны типы создаваемых объектов
- Нужно дать пользователям возможность расширять систему
- Нужно экономить ресурсы через повторное использование объектов
...
| **Создаёт** | Один тип продукта | Семейство продуктов |
| **Расширение** | Через подклассы Creator | Через новые Factory классы |
| **Связанность** | Сильная (наследование) | Слабая (интерфейс) |
| **Гибкость** | Средняя | Высокая |



### # Проблема Factory Method при создании семейств

Рассмотрим пример с диалоговыми окнами. Представьте, что помимо кнопки нам нужно создать ещё текстовое поле и полосу прокрутки. Все эти элементы должны быть в одном стиле!
...
После таких изменений в каждом Creator'е нам нужно будет добавить реализацию этих методов. Иными словами пробежаться во всем таким классам и вручную из изменить. Очевидно, что если таких классов будет достаточно много, то сделать это будет крайне тяжело.

Иными словами фабричный метод не работает, когда нам нужно создать не один объект, а семейство каких-то связанных между собой объектов.
...
(В примерах выше это была фабрика всяких UI элементов, такие как кнопок)
...
- ✅ Изолирует конкретные классы продуктов от клиентского кода
- ✅ Упрощает замену семейств продуктов
- ✅ Соблюдает принцип Open/Closed

**Недостатки:**
- ❌ Усложняет код из-за множества дополнительных классов
- ❌ Требует изменения интерфейса фабрики при добавлении новых типов продуктов

**Когда использовать:**
- Система должна работать с семействами связанных продуктов
- Нужно гарантировать совместимость продуктов
- Клиент не должен зависеть от конкретных классов

**Отличие от Factory Method:**
- Factory Method создаёт **один тип** объектов через **наследование**
- Abstract Factory создаёт **семейство** объектов через **композицию**
...
Далеко не все аргументы всегда используются и приходится делать их null. Либо создаётся проблема "телескопических конструкторов", Иными словами матрёшки из конструкторов:
...
Иными словами мы вынесли логику создания из чего-то цельного в "как бы лего конструктор", куда мы при желании можешь в любой момент присоединить что-то новое.
...
Иными словами просто список каких-то заказов:
...
**Практический пример**: Система заказов интернет-магазина

Представьте заказ с множеством опциональных параметров: комментарии, телефон получателя, время доставки, подарочная упаковка и так далее. Builder решает эту проблему изящно.

```csharp
// Строитель
// Builder для создания заказов
...
Рассмотрим два вида билдеров:

**Типы строителей:**



**Сравнение типов билдеров:**

| Тип | Назначение | Пример использования |
|-----|------------|---------------------|
| **Convenience Builder** | Упрощение создания с многими параметрами | Объекты с 5+ параметрами, многие со значениями по умолчанию |
| **Stateful Constructor** | Валидация при создании | Заказы с ограничениями, формы с проверками |
| **Interface-Driven Builder** | Типобезопасное пошаговое создание | Объекты с обязательной последовательностью шагов |
...
Строитель конструирует объекты пошагово, а не за один проход. Более того, шаги строительства можно выполнять рекурсивно. А без этого не построить древовидную структуру, такие как Компоновщика.
...
Нам нужен какой-то *чертёж*. Это и есть суть паттерна Прототип (Prototype). Prototype — это паттерн, когда объект сам знает, как себя копировать. Он переносит ответственность за создание копии с клиентского кода на сам объект-прототип.
...
Тем не менее идею с прототипами можно развить куда сильнее. Рассмотрим несколько разных подходов:

### # Shallow copy (поверхностное копирование) (поверхностное копирование)
...
Иными словами есть проблема: мы получили два разных объекта Prototype, но они оба ссылаются на одну и ту же коллекцию в памяти. Если мы изменим эту коллекцию в оригинале (предположим, она не IReadOnlyCollection), изменения увидит и копия.
...
### # Deep copy (глубокое копирование) (глубокое копирование)
...
Рассмотрим ещё несколько примеров этой проблемы:
...
Иными словами у нас всё равно неправильная сигнатура
...
Иными словами теперь можно использовать так:
...
Если у вас есть иерархия? Например, SecondPrototype наследует Prototype:
...
Рассмотрим ещё несколько примеров с рекурсивными дженериками и их проблемами. Для этого вернёмся к примеру с фигурами:
...
Иными словами ещё раз проблема с необобщённым интерфейсом:
...
1. **Logger (Логгер)** — единая точка записи логов
2. **Configuration Manager** — хранилище настроек приложения
3. **Database Connection Pool** — управление подключениями к БД
4. **Cache Manager** — централизованное управление кэшем
5. **Thread Pool** — управление пулом потоков

**Важно**: Singleton должен быть простым объектом, предоставляющим доступ к ресурсу, а не содержать сложную бизнес-логику.
...
**⚠️ Внимание: Singleton как анти-паттерн**

Многие разработчики считают Singleton **анти-паттерном** из-за:
- Глобального состояния (нарушает инкапсуляцию)
- Сложности тестирования
- Скрытых зависимостей

**Современная альтернатива**: Использование **Dependency Injection (DI)** контейнеров, которые управляют жизненным циклом объектов более явно и тестируемо.
