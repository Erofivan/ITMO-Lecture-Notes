...
public abstract class Dialog
{
    public void Render()
    {
        // Нам нужно создать ДВА элемента, и они должны быть из ОДНОГО семейства
    ...
    protected abstract IButton CreateButton();
    protected abstract ITextBox CreateTextBox(); // Второй абстрактный метод
}
```
...
```csharp
// С каждым новым элементом добавляется ещё один абстрактный метод
public abstract class Dialog
{
    public void Render()
    {
    ...
    // Уже три абстрактных метода
    protected abstract IButton CreateButton();
    protected abstract ITextBox CreateTextBox(); 
    protected abstract IScrollBar CreateScrollBar(); // Третий абстрактный метод
}
```
...
2. **Вариации стилей**: каждый продукт представлен в трёх стилях — *Ар-деко* (Art Deco), *Модерн* (Modern), *Викторианский* (Victorian).
...
1. **Согласованность семейства**: все предметы мебели в одном заказе должны быть одного стиля. Клиенты будут недовольны, если получат кресло в стиле модерн вместе с викторианским диваном.
...
public class ModernArmchair : IArmchair
{
    public void SitOn() 
    {
        Console.WriteLine("Вы сидите на минималистичном кресле в стиле модерн.");
    }
}
...
public class ModernSofa : ISofa
{
    public void LieOn() 
    {
        Console.WriteLine("Вы лежите на лаконичном диване модерн с чистыми линиями.");
    }
}
...
public class ModernCoffeeTable : ICoffeeTable
{
    public void PutCoffee() 
    {
        Console.WriteLine("Вы ставите чашку на функциональный столик модерн из натурального дерева.");
    }
}
...
Паттерн выделяет ответственность за создание объектов в отдельные классы-фабрики. Клиентский код отвечает за использование объектов, а не за их создание.

### Open/Closed Principle (OCP)

Система открыта для расширения (можно добавлять новые фабрики и продукты) и закрыта для модификации (существующий клиентский код не меняется).

### Liskov Substitution Principle (LSP)

Все конкретные фабрики взаимозаменяемы, так как реализуют один интерфейс. Клиентский код работает корректно с любой фабрикой.

### Interface Segregation Principle (ISP)

Интерфейс фабрики содержит только методы создания продуктов, без лишних зависимостей.

### Dependency Inversion Principle (DIP)

Клиентский код зависит от абстракций (`IFactory`, `IProduct`), а не от конкретных реализаций. Это основа паттерна.
